...
class CNNTransformerModel(nn.Module):
    def __init__(self, in_channels=3, cnn_out_dim=128, num_classes=2, max_len=10001):
        ...
        self.time_weights = nn.Parameter(torch.randn(max_len))
        self.max_len = max_len
    def forward(self, x):  # x: (B, T, C, N, N)
        B, T, C, N, N_ = x.shape
        assert T <= self.max_len, f"Input T={T} exceeds max_len={self.max_len}"
        x = self.encoder(x)       # (B, T, cnn_out_dim)
        norm_weights = torch.softmax(self.time_weights[:T], dim=0).unsqueeze(0).unsqueeze(-1)  # (1, T, 1)
        x = x * norm_weights  # (B, T, cnn_out_dim) × (B, T, 1)
        out = self.temporal(x)    # (B, num_classes)
        return out, self.time_weights
