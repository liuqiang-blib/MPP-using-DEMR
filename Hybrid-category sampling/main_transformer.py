...
def task_evaluate_metrics(model, loader, criterion, device,num):
    ...
    with torch.no_grad():
        ...
        for x_batch, y_batch,rmsd_batch in loader:
            ...
            running_loss += loss.item() * x_batch.size(0)
            preds = torch.argmax(logits, dim=1)
            probs = torch.softmax(logits, dim=1)[:, 1]  # 二分类取正类概率
    ...
    for task in task_list:
        ...
        for freq in freq_list:
            ...
            for num in num_list:
                ...
                loader = MoleculeDataloader(features, labels, mol_id_list=mol_id_list, rmsd_dir=rmsd_dir,
                                                 extract_config={"high": num, "mid": num, "low": num}, batch_size=16,
                                                 num_workers=4)
                ...
                model = TransformerClassifier(num_classes=2, max_len= 3 * num)
                ...
                path = "/data/rmsdtest/others/transformer/" + str(num) + "_" 
                ...
                results_metrics, model_weights, all_rmsd = task_evaluate_metrics(model, train_loader, criterion,
                                                                                     device, num)
                df_results = pd.concat([pd.DataFrame(np.array(mol_id_list), columns=['mol_id_list']),
                                        pd.DataFrame(model_weights.reshape(len(model_weights), 1),
                                                     columns=['model_weights']),
                ...
                results_metrics, model_weights, all_rmsd = task_evaluate_metrics(model, test_loader, criterion,
                                                                                     device, num)
