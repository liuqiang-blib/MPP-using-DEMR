...
def task_evaluate_metrics(model, loader, criterion, device,num):
    model.eval()
    running_loss = 0.0
    running_correct = 0
    running_total = 0
    ...
    with torch.no_grad():
        ...
        for x_batch, y_batch,rmsd_batch in loader:
            ...
            running_correct += (preds == y_batch).sum().item()
            running_total += y_batch.size(0)
            model_weights.append(weights.cpu().numpy())
    ...
    return metrics,np.array(model_weights[0][0:3*num]),torch.cat(all_rmsd, dim=0).numpy()
...
if __name__ == "__main__":
    ...
    freq_list = [0]
    num_list = [2,5,10,20,50,100,1000]
    ...
    for task in task_list:
        for freq in freq_list:
            for num in num_list:
                ...
                data_loader = MoleculeDataloader(features, labels, mol_id_list=mol_id_list, rmsd_dir=rmsd_dir, extract_config = {"high": num , "mid": num , "low": num }, batch_size=16, num_workers=4)
                del features
                gc.collect()
                ...
                model = CNNTransformerModel(num_classes=2, max_len= num*3)
                ...
                path  = "/data/rmsdtest/others/cnntransformer/" + str(num) + "_"   # 只需要更改这一个
                ...
                results_metrics,model_weights,all_rmsd = task_evaluate_metrics(model, train_data, criterion, device,num)
                df_results = pd.concat([pd.DataFrame(np.array(mol_id_list), columns=['mol_id_list']),
                                pd.DataFrame(model_weights.reshape(len(model_weights),1), columns=['model_weights']),
                                pd.DataFrame(all_rmsd), ],
                               axis=1)
                df_results .to_csv( path + task +  "weights.csv", index=False)

                test_loss,test_acc = evaluate(model, test_data, criterion)
                print(f"\n=== Test Loss: {test_loss:.4f} === | Test acc: {test_acc:.4f} ===")
               
                results_metrics,model_weights,all_rmsd = task_evaluate_metrics(model, test_data, criterion, device,num)
