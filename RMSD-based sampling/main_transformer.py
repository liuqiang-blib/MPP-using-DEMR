...
def train_model(model, train_loader, val_loader, epochs, optimizer, criterion, save_path):
    ...
    for epoch in range(epochs):
        ...
        for x_batch, y_batch, rmsd_batch in tqdm(train_loader, desc=f"Epoch {epoch}/{epochs} [Train]"):
            ...
            rmsd_batch = rmsd_batch.to(device)
            optimizer.zero_grad()
            output = model(x_batch,rmsd_weights=rmsd_batch)
        ...
        for freq in freq_list:
            for mode in mode_list:
                for frame_count in frame_count_list:
                    df = pd.read_csv(f"/data/features_sampling_freq_{freq}/{task}_molecule_metadata.csv")
                    features = np.load(f"/data/features_sampling_freq_{freq}/{task}_molecule_features.npy", mmap_mode='r')
                    labels = df['label'].values
                    mol_id_list = df['mol_id'].values
                    rmsd_dir  = "/dataMD_analysis"
                    ...
                    loader = MoleculeDataloader(features, labels, mol_id_list=mol_id_list, rmsd_dir=rmsd_dir, mode=mode, frame_count=frame_count, batch_size=16, num_workers=4)
                    ...
                    model = TransformerClassifier(num_classes=2, max_len= frame_count)
                    ...
                    path  = "/data/RMSD_select_weighting/TOX21/results_transformer_norm/" + mode  +"/"
                    ...
                    history = train_model(model, train_loader, val_loader, epochs=100, optimizer=optimizer, criterion=criterion, save_path=save_path)
                    end = time.time()
