...
class TransformerClassifier(nn.Module):
    def __init__(self, input_dim=192, d_model=64, nhead=4, num_layers=6, num_classes=2, dropout=0.1, max_len=10001):
        super().__init__()
        self.input_proj = nn.Linear(input_dim, d_model)  # 将原始flatten后的图像映射到d_model维度
        self.pos_encoder = PositionalEncoding(max_len=max_len, d_model=d_model)

        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dropout=dropout)  # 不加 batch_first
    ...
    def forward(self, x, rmsd_weights=None):  # x: (B, T, F)
        x = self.input_proj(x)  # (B, T, d_model)
        if rmsd_weights is not None:
            # 添加一维以匹配最后的 d_model 维度，再广播乘法
            x = x * rmsd_weights.unsqueeze(-1)  # (B, T, d_model)
