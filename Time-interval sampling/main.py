    ...
    for task in task_list:
        for freq in freq_list:
            for num in range(3):
                ...
                df = pd.read_csv("./data_" + str(freq) +  "/" + task + "_molecule_metadata.csv")
                features = np.load("./data_" + str(freq)  +  "/"  + task + "_molecule_features.npy", mmap_mode='r')
                ...
                data_loader = MoleculeDataloader(features, labels, batch_size=16, num_workers=4)
                del features
                gc.collect()
                ...
                model = CNNTransformerModel(num_classes=2, max_len=10001 if freq == 0 else (10001 // freq + 1))
                if torch.cuda.device_count() > 1:
                    print(f"Using {torch.cuda.device_count()} GPU for training")
                    model = torch.nn.DataParallel(model)
                model = model.to(device)
                print("model run in：", model.device_ids if isinstance(model, torch.nn.DataParallel) else device)

                optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)


                path  = "./results/" + str(freq)  +"/"
                save_path =  path + task +  str(num) +  "_best_model.pt"
                start_time = time.time()
                train_history = train_model(model, train_data, val_data, 200, optimizer=optimizer, criterion=criterion, save_path=save_path)
                end_time = time.time()
                ...
                if os.path.exists( path  + task + "train_history.csv"):
                    df.to_csv( path + task + str(num) + "_train_history.csv", mode='a', index=False, header=False)
                else:
                    df.to_csv( path + task + str(num) + "_train_history.csv", mode='w', index=False, header=True)
                ...
                with open( path + task + str(num) + "_test_results.txt", "w") as f:
